...
/**
 * 老人请假申请主对象 lc_elder_leave
 *
 * @author ruoyi
 * @date 2026-05-24
 */
public class LcElderLeave extends BaseEntity
{
    ...
    /** 护理员姓名，多个用逗号分隔 */
    @Excel(name = "护理员姓名，多个用逗号分隔")
    private String nurseNames;

    /** 陪同人类型：家属、护理人员、其他、无 */
    @Excel(name = "陪同人类型：家属、护理人员、其他、无")
    ...
    @Excel(name = "业务状态：draft草稿、approving审批中、approved已通过、rejected已拒绝、revoked已撤回、leaving请假中、returned已返回、timeout超时未归、cancelled已作废")
    ...
    /** 实际请假天数（冗余字段，便于列表查询） */
    @Excel(name = "实际请假天数", readConverterExp = "冗=余字段，便于列表查询")
