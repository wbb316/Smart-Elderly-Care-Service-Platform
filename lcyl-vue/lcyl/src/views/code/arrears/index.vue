    ...
    <el-table v-loading="loading" :data="arrearsList">
      ...
      <el-table-column label="欠费金额(元)" align="center" min-width="140">
        <template #default="scope">
          <span class="arrears-amount">{{ formatMoney(scope.row.arrearsAmount) }}</span>
        </template>
      </el-table-column>
...
function handleQuery() {
  ...
  queryParams.value.arrearsOnly = "1"
  getList()
}
...
function handleExport() {
  proxy.download(
    ...
    `arrears_${new Date().getTime()}.xlsx`
  )
}
